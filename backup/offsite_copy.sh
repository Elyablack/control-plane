...
ssh "$TARGET_HOST" "ls -t ${TARGET_DIR}/vps-backup-*.tar.gz 2>/dev/null | tail -n +8 | xargs -r rm -f && ls -t ${TARGET_DIR}/vps-backup-*.tar.gz.sha256 2>/dev/null | tail -n +8 | xargs -r rm -f"
